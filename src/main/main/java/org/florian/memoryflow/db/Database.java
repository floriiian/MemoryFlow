...
public class Database {
    ...
    public HashMap<String, Object[]> getCategoriesByOwner(String user_id) {
        String sql = "SELECT category, COUNT(question), visibility FROM flashcards WHERE user_id = ? GROUP BY category";
        try (PreparedStatement preparedStmt = CONNECTION.prepareStatement(sql)) {
            preparedStmt.setString(1, user_id);
            return getCategorieHashmap(preparedStmt);
        } catch (Exception e) {
            LOGGER.debug(e);
            return null;
        }
    ...
    public ArrayList<Streak> getStreaks() {
        String sql = "SELECT user_id, streak, date, claimed FROM progress";
        try (PreparedStatement preparedStmt = CONNECTION.prepareStatement(sql);
            ...
            while (results.next()) {
                ...
                String claimed = results.getString("claimed");
                resultList.add(new Streak(user_id, streak, last_completion, claimed));
            }
            return resultList;
        } catch (SQLException e) {
    ...
    private HashMap<String, Object[]> getCategorieHashmap(PreparedStatement preparedStmt) throws SQLException {
        try (ResultSet results = preparedStmt.executeQuery()) {
            ...
            while (results.next()) {
                ...
                String visibility = results.getString(3);
                resultList.put(category, new Object[]{count, visibility.equals("true")});
            }
            return resultList;
        }
    }
        ...
        try (PreparedStatement stmt = CONNECTION.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Object[] values = { rs.getInt("entry_count"), rs.getInt("card_id") };
                    resultMap.put(category, values);
                }
            }
        } catch (SQLException e) {
            LOGGER.debug(e);
    ...
    public  void updateValueWithTwoConditions(String table, String column, String where, String whereValue, String where2, String whereValue2, Object value) {
        String sql = "UPDATE " + table + " SET " + column + " = ? WHERE " + where + " = ? AND " + where2 + " = ?";
        try {
            PreparedStatement preparedStmt = CONNECTION.prepareStatement(sql);
            preparedStmt.setObject(1, value);
            preparedStmt.setString(2, whereValue);
            preparedStmt.setString(3, whereValue2);
